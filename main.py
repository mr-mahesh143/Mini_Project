...
label = tk.Label(window, text="REAL TIME LECTURE VISUAL GENERATION", height=1, width=WIDTH)
...
input_field = tk.Text(text_div, bd=1, relief="solid", width=127, height=4)
...
submit_button = tk.Button(text_div, text="START", width=11, height=3, command=lambda: append_lecture())
...
mic_image = Image.open(r'C:\Users\Mahesh\Documents\ALL DOCUMENTS\mini project\RTLVG source code with plagarism\v1.0\mic.png')
mic_image = mic_image.resize((80, 90), resample=Image.LANCZOS)
mic_icon = ImageTk.PhotoImage(mic_image)

# Create a button with the microphone icon image
mic_label = tk.Button(text_div, image=mic_icon, width=80, height=80, command=lambda: toggle_listening(input_field))
